...
use crate::{
    ...
    dto::request::auth::{
        ForgetPasswordRequest, LoginRequest, RefreshTokensRequest, RegisterRequest,
        ResetPasswordRequest, VerifyUserRequest,
    },
...
pub fn validate_refresh_tokens_payload(payload: &RefreshTokensRequest) -> AppResult<()> {
    if payload.access_token.len() == 0 {
        return Err(
            ValidationErrorKind::ValidationFailed("access_token is empty".to_string()).into(),
        );
    }
    if payload.refresh_token.len() == 0 {
        return Err(
            ValidationErrorKind::ValidationFailed("refresh_token is empty".to_string()).into(),
        );
    }
    Ok(())
}
